...
const profile = async (req, res) => {
    try {
        ...
        // Filter out the fileUrl, userId from the files
        if (user && user.files) {
            user.files = user.files.map(({fileUrl, userId, ...file}) => file);
        }

        const {password, createdAt, emailConfirmed, confirmationToken, ...userToSend} = user;
